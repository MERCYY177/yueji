(() => {
  ...
  async function persistSplitWeReadBook(book){if(!book?.weReadBookId)return;const sessions=state.sessions.filter(s=>s.source==='weread'&&String(s.bookKey||'')===String(book.key));await persistSyncIncrement({books:[book],sessions});diagnosticStage('split-book-complete',{bookId:String(book.weReadBookId),bookKey:String(book.key),sessions:sessions.length})}
  ...
  function restoreCachedWeReadBook(found,incoming){
    const canonicalKey=String(found.key||incoming.key||''),localSources=(found.sources||[]).filter(Boolean),hasMoon=localSources.includes('moon'),hasManualProgress=found.progressSource==='manual'||found.manualProgress;
    const local=hasMoon?{file:found.file,moonPath:found.moonPath,moonId:found.moonId,cover:found.cover,minutes:found.minutes,words:found.words,days:found.days}:{};
    if(hasManualProgress)Object.assign(local,{progress:found.progress,progressSource:found.progressSource,manualProgress:found.manualProgress,status:found.status});
  ...
  async function recoverSyncSnapshot(){let rows=[];try{rows=await syncDbRequest(store=>store.getAll())||[]}catch(error){console.warn('微信同步数据库读取失败',error);return false}if(!rows.length)return false;const hidden=new Set((state.hiddenWeReadBookIds||[]).map(String)),meta=rows.find(x=>x.id==='meta'),shelf=rows.find(x=>x.id==='shelf'),keyMap=new Map();if(meta?.weRead)state.weRead={...state.weRead,...meta.weRead,syncState:meta.weRead.syncState==='running'?'paused':meta.weRead.syncState};if(shelf?.value)state.weRead.shelfBooks=shelf.value.filter(b=>!hidden.has(String(b?.bookId||'')));const byId=new Map(state.books.filter(b=>b.weReadBookId).map(b=>[String(b.weReadBookId),b])),byKey=new Map(state.books.map(b=>[String(b.key),b]));for(const row of rows){if(row.kind!=='book'||!row.value)continue;const incoming=row.value;if(incoming.weReadBookId&&hidden.has(String(incoming.weReadBookId)))continue;const found=incoming.weReadBookId?byId.get(String(incoming.weReadBookId)):byKey.get(String(incoming.key));if(found){if(incoming.key&&String(incoming.key)!==String(found.key))keyMap.set(String(incoming.key),String(found.key));restoreCachedWeReadBook(found,incoming)}else{state.books.push(incoming);byKey.set(String(incoming.key),incoming);if(incoming.weReadBookId)byId.set(String(incoming.weReadBookId),incoming)}}const resolveKey=key=>{let value=String(key||''),guard=0;while(keyMap.has(value)&&guard++<20)value=keyMap.get(value);return value};const savedSessions=rows.filter(x=>x.kind==='session'&&x.value).map(x=>{const value=x.value;return value.bookKey?{...value,bookKey:resolveKey(value.bookKey)}:value}),sessionIds=new Set(savedSessions.map(x=>x.id));state.sessions=state.sessions.filter(s=>s.source!=='weread'||!sessionIds.has(s.id)).concat(savedSessions);state.weRead.daily={};savedSessions.forEach(s=>{if(s.date)state.weRead.daily[s.date]=n(s.seconds)||n(s.minutes)*60});rebuildBookIndexes();return true}
  ...
  function injectSettings() {
    ...
    document.getElementById('wereadProgressRefreshBtn').addEventListener('click',async()=>{
      ...
      state.weRead.progressCursor=0;state.weRead.progressDone=false;state.weRead.syncPhase='progress';state.weRead.autoRetryBlocked=false;delete state.weRead.progressLastResult;
      await syncWeRead({mode:'quick',manual:true});
    });
  ...
  function progressEvidenceSummary(){const summary={accepted:0,noDate:0,baseline:0,other:0},without=[];state.books.filter(b=>b.sources?.includes('weread')).forEach(b=>{const evidence=b.weReadProgressEvidence;if(evidence?.accepted){summary.accepted++;return}const reason=String(evidence?.reason||'');if(/没有返回最后阅读时间/.test(reason))summary.noDate++;else if(/进度基线/.test(reason))summary.baseline++;else summary.other++;if(without.length<5)without.push(b.title||'未命名')});return{summary,without}}
  function syncReportHtml(){const r=state.weRead.lastReport;if(!r)return'<div class="wr-evidence-note"><b>微信圆圈如何产生</b><span>只有逐书进度接口返回了可信的最后阅读时间，或相邻两次同步确认进度/时长增加时，才生成圆圈。仅在书架里出现、只有总阅读时长或当天没有变化，都不会伪造圆圈。</span></div>';const at=new Date(r.completedAt).toLocaleString('zh-CN',{hour12:false}),s=r.evidenceSummary||{},without=(r.noCircleTitles||[]).map(title=>`《${esc(title)}》`).join('、');return`<div class="wr-sync-report"><b>最近一次完整同步</b><span>${esc(at)}</span><div><em>${n(r.shelfBooks)} 本书架</em><em>${n(r.readDays)} 个阅读日</em><em>${n(r.progressBooks)} 本已查进度</em><em>${n(r.circleBooks)} 本有微信圆圈</em><em>${n(r.notebookBooks)} 本有笔记目录</em></div></div><details class="wr-evidence-note"><summary><b>查看圆圈证据说明</b></summary><span>${n(r.detailBooks)} 本拿到了逐书日期；${n(s.accepted)} 本确认活动，${n(s.noDate)} 本没有逐书日期，${n(s.baseline)} 本目前只是进度基线，${n(s.other)} 本没有可用变化。书架更新时间不等于当天阅读，不会单独生成圆圈。${without?`<br>本次未生成圆圈的示例：${without}${n(r.progressBooks)-n(r.circleBooks)>5?'等':''}。`:''}</span></details>`}
  function updateWeReadProgressPanel(){const panel=document.getElementById('wereadProgressPanel');if(!panel)return;const progressTotal=n(state.weRead.progressTotal),notesTotal=n(state.weRead.notebooksTotal),progressDone=state.weRead.progressDone?progressTotal:n(state.weRead.progressCursor),notesDone=state.weRead.notesDone?notesTotal:n(state.weRead.notesCursor),last=state.weRead.lastSync?new Date(state.weRead.lastSync).toLocaleString('zh-CN',{hour12:false}):'尚未完成',monthProgress=!state.weRead.statsDone&&state.weRead.statsSyncYear?`${Math.min(n(state.weRead.statsMonthCursor),12)} 月已保存`:`${n(state.weRead.statsDays)} 天`,circleBooks=state.books.filter(b=>b.sources?.includes('weread')&&(window.yuejiVerifiedWeReadActivityDates?.(b)||[]).length).length,detailBooks=state.books.filter(b=>b.sources?.includes('weread')&&b.weReadProgressEvidence?.date).length,lastProgress=state.weRead.progressLastResult,lastProgressText=lastProgress?`<small class="wr-diagnostic">最近读取：《${esc(lastProgress.title||'未命名')}》 · ${esc(lastProgress.reason||'已保存')}</small>`:'';let diagnostic='';try{const row=JSON.parse(localStorage.getItem(SYNC_DIAGNOSTIC_KEY)||'null');if(row?.stage&&!String(row.stage).endsWith('complete'))diagnostic=`<small class="wr-diagnostic">上次中断位置：${esc(row.stage)} · ${esc(new Date(row.at).toLocaleString('zh-CN',{hour12:false}))}</small>`}catch{}panel.innerHTML=`<div class="wr-progress-head"><b>${esc(syncStateLabel())} · ${esc(syncPhaseLabel())}</b><span>最后更新：${esc(last)}</span></div><div class="wr-progress-grid"><div class="wr-progress-item"><span>书架</span><strong>${n(state.weRead.shelfTotal)} 本</strong></div><div class="wr-progress-item"><span>阅读统计</span><strong>${esc(monthProgress)}</strong></div><div class="wr-progress-item"><span>阅读进度</span><strong>${progressDone} / ${progressTotal || '待读取'}</strong></div><div class="wr-progress-item"><span>微信圆圈</span><strong>${circleBooks} 本 / ${detailBooks} 本有时间</strong></div><div class="wr-progress-item"><span>书摘书目</span><strong>${notesDone} / ${notesTotal || '待读取'}</strong></div></div>${lastProgressText}${diagnostic}${syncReportHtml()}`}
  ...
  async function syncProgressBatch(books, ctx) {
    ...
    const batch=started.slice(start,start+PROGRESS_BATCH_SIZE); let completed=0,changedBooks=[];
    for(const b of batch){
      assertSyncActive(ctx); setWeReadStatus(`正在分批补阅读进度 ${start+completed+1}/${started.length}……`);
      try{const p=await wereadCall('/book/getprogress',{bookId:b.bookId},ctx),merged=mergeBookFromWeRead(b,p),evidence=merged?.weReadProgressEvidence;if(merged)changedBooks.push(merged);state.weRead.progressLastResult={bookId:String(b.bookId||''),title:b.title||b.name||merged?.title||'未命名',reason:evidence?.reason||'接口没有返回可用的阅读证据',accepted:Boolean(evidence?.accepted)}}catch(e){if(/^SYNC_/.test(e.message))throw e;state.weRead.progressLastResult={bookId:String(b.bookId||''),title:b.title||b.name||'未命名',reason:`读取失败：${String(e?.message||e)}`,accepted:false};console.warn('WeRead progress skipped',e)}
  ...
  async function syncWeRead({mode='quick',manual=false,resetDetails=false}={}) {
    ...
    try{
      ...
      else if(phase==='progress'){const progress=await syncProgressBatch(books,ctx);changedBooks=progress.changedBooks||[];state.weRead.syncPhase=progress.done?'notebooks':'progress';extra=progress.done?`阅读进度 ${progress.completed}/${progress.total} 已完成。再次点击读取笔记目录第一页。`:`已保存阅读进度 ${progress.completed}/${progress.total}。再次点击读取下一本。`}
