(()=>{
  ...
  async function seed(db){let tx=db.transaction(['progress','notebooks','meta'],'readwrite'),p=done(tx,'测试队列初始化失败'),progress=tx.objectStore('progress'),notes=tx.objectStore('notebooks');for(let i=0;i<PROGRESS_TOTAL;i++)progress.put({id:i,bookId:`p-${i}`});for(let i=0;i<NOTEBOOK_TOTAL;i++)notes.put({id:i,bookId:`n-${i}`});tx.objectStore('meta').put({key:'state',progressCursor:0,notesCursor:0,crashes:0});await p}
  ...
  function testLock(){const a='A',b='B',now=Date.now(),live={owner:a,expiresAt:now+10000};localStorage.setItem(LOCK,JSON.stringify(live));const read=()=>{try{return JSON.parse(localStorage.getItem(LOCK)||'null')}catch{return null}};const held=x=>x&&Number(x.expiresAt)>Date.now();if(!(held(read())&&read().owner!==b))throw new Error('跨页面锁没有阻止第二个 owner');localStorage.setItem(LOCK,JSON.stringify({owner:a,expiresAt:Date.now()-1}));if(held(read()))throw new Error('过期锁没有释放');localStorage.removeItem(LOCK);if(typeof window.yuejiP0SyncLeaseInfo!=='function'||!window.__yuejiP0SyncGuardV2Installed)throw new Error('原子同步锁模块没有正确安装')}
  function auditListeners(){
    const names=['yueji-p0-sync-guard-v2.js','yueji-p0-resume.js','yueji-p0-failures.js','yueji-p0-inflight.js','yueji-p0-chain-test.js','yueji-p0-runtime-audit.js','yueji-p0-auto-lock.js'];
    const duplicates=[];for(const name of names){const c=[...document.scripts].filter(s=>String(s.src).includes(name)).length;if(c!==1)duplicates.push(`${name} × ${c}`)}
    ...
    if(duplicates.length||duplicateIds.length)throw new Error(`检测到模块缺失/重复：${[...duplicates,...duplicateIds].join('、')}`);
    const flags=['__yuejiP0SyncGuardV2Installed','__yuejiP0FailuresInstalled','__yuejiP0InflightInstalled','__yuejiP0AutoLockInstalled','__yuejiP0ChainTestInstalled','__yuejiP0RuntimeAuditInstalled'];
    const missing=flags.filter(k=>!window[k]);if(missing.length)throw new Error(`P0 模块安装标记缺失：${missing.join('、')}`);
    const health=window.yuejiP0RuntimeAudit?.();if(health&&!health.healthy)throw new Error(`运行时健康检查失败：${health.errors?.[0]||'未知错误'}`);
    return names.length;
  }

  async function run(){
    const out=document.getElementById('diagnosticResult'),btn=document.getElementById('runSyncChainStress');if(!out||!btn)return;btn.disabled=true;out.className='diagnostic-result';out.textContent='正在运行 P0 同步全链路压力测试……';
    const lines=['P0 同步链路压力测试（独立临时数据库，不读取或修改真实阅读记录）'];let db=null,start=performance.now();
    try{
      await deleteDb();db=await openDb();await seed(db);lines.push(`初始化：${PROGRESS_TOTAL} 本进度 + ${NOTEBOOK_TOTAL} 本书摘目录`);db=await processProgress(db,lines);db=await processNotes(db,lines);
      const failedBefore=await count(db,'failures'),retried=await retryFailures(db);lines.push(`故障注入：${failedBefore} 项；重试处理：${retried} 项`);db=await testInflight(db);testLock();const modules=auditListeners();
      ...
      if(meta.progressCursor!==PROGRESS_TOTAL)throw new Error(`进度断点错误：${meta.progressCursor}/${PROGRESS_TOTAL}`);if(meta.notesCursor!==NOTEBOOK_TOTAL)throw new Error(`书摘断点错误：${meta.notesCursor}/${NOTEBOOK_TOTAL}`);if(progressDone!==PROGRESS_TOTAL)throw new Error(`进度完成数错误：${progressDone}/${PROGRESS_TOTAL}`);if(notesDone!==NOTEBOOK_TOTAL)throw new Error(`书摘完成数错误：${notesDone}/${NOTEBOOK_TOTAL}`);if(failures!==0)throw new Error(`失败项重试后仍残留 ${failures} 项`);if(highlights!==expectedHighlights)throw new Error(`稳定 ID 去重异常：${highlights}/${expectedHighlights}`);if(inflight!==0)throw new Error(`in-flight 恢复后仍残留 ${inflight} 项`);
      lines.push(`断点恢复：通过（模拟 ${meta.crashes||0} 次页面中断）`,`稳定 ID 去重：通过（${highlights.toLocaleString()} 条唯一书摘）`,'失败项优先重试：通过','in-flight 成功响应恢复：通过','跨页面锁模块：通过',`运行时模块检查：通过（${modules} 个 P0 脚本）`,`总耗时：${Math.round(performance.now()-start)} ms`,'结论：P0 同步链路压力测试通过。');out.className='diagnostic-result ok';out.textContent=lines.join('\n');
