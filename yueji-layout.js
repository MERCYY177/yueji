...
function installHome(){
  ...
  wrap.addEventListener('click',e=>{
    ...
    if(g){switchPage(g.dataset.homeGo);return}
    const st=e.target.closest('[data-home-stats]');
    if(st){switchPage('analytics');showStatsTab(st.dataset.homeStats);}
  });
}
...
function renderHomeDashboard(){
  ...
  state.sessions.forEach(s=>{if(s.date?.startsWith(`${yr}-`))yearDays.add(s.date)});
  Object.values(state.journals||{}).forEach(j=>{if(j.date?.startsWith(`${yr}-`)&&j.read)yearDays.add(j.date)});
  document.getElementById('homeFootMeta').innerHTML=`<b>${yearDays.size} 天</b><span>${yr} 年已留下阅读记录</span>`;
  const cells=[];
  for(let i=55;i>=0;i--){
    const d=addDays(today,-i),k=dateKey(d),dd=dayData(k),level=dd.read?Math.min(.9,.25+(dd.minutes||1)/120):0;
    cells.push(`<i class="home-foot-cell${dd.read?' read':''}" style="--level:${level}" title="${esc(k)}${dd.read?` · ${dd.minutes} 分钟`:''}"></i>`);
  }
...
function installLibrary(){
  ...
  p.className='page';p.dataset.page='library';
  ...
  search.className='search';search.placeholder='搜索书名 / 作者';
  document.getElementById('libraryTopRow').appendChild(search);
  const seg=document.getElementById('librarySeg');
  seg.className='seg';
  seg.innerHTML='<button class="active" data-filter="all">全部</button><button data-filter="reading">阅读中</button><button data-filter="done">读完</button><button data-filter="unread">未开始</button>';
  document.getElementById('librarySegHolder').replaceWith(seg);
  const count=document.getElementById('libraryCount');
  document.getElementById('libraryCountHolder').replaceWith(count);count.className='library-count';
  const shelf=document.getElementById('bookshelf');
  document.getElementById('libraryDisplay').appendChild(shelf);
  const table=document.getElementById('bookTable');table.hidden=true;

  search.oninput=()=>renderLibrary();
  seg.onclick=e=>{const b=e.target.closest('[data-filter]');if(!b)return;libraryFilterLocal=b.dataset.filter;seg.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===b));renderLibrary()};
  document.getElementById('libraryViewSeg').onclick=e=>{const b=e.target.closest('[data-library-view]');if(!b)return;libraryView=b.dataset.libraryView;b.parentElement.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===b));renderLibrary()};
}
...
function renderLibraryLayout(){
  ...
  if(libraryView==='wall'){
    ...
    const rows=[];for(let i=0;i<arr.length;i+=14)rows.push(arr.slice(i,i+14));
...
function streakStats(){
  ...
  arr.forEach(k=>{if(prev&&dateKey(addDays(parseDate(prev),1))===k)run++;else run=1;max=Math.max(max,run);prev=k});
  let anchor=set.has(todayKey)?todayKey:dateKey(addDays(today,-1));if(!set.has(anchor))return{current:0,max};
  let current=0,d=parseDate(anchor);while(set.has(dateKey(d))){current++;d=addDays(d,-1)}
  return{current,max};
}
...
function installStatsStructure(){
  const analytics=document.querySelector('.page[data-page="analytics"]');if(!analytics||document.getElementById('statsTabs'))return;
  const head=analytics.querySelector('.analytics-head');
  const sub=head?.querySelector('.section-sub');if(sub)sub.textContent='汇总微信读书、静读天下与手动记录。';
  const tabs=document.createElement('div');tabs.className='stats-tabs';tabs.id='statsTabs';tabs.innerHTML='<button class="active" data-stats-tab="overview">总览</button><button data-stats-tab="evolution">演化</button><button data-stats-tab="month">月报</button><button data-stats-tab="year">年报</button>';
  head.after(tabs);
  ['overview','evolution','month','year'].forEach((n,i)=>{const p=document.createElement('div');p.className='stats-panel';p.id=`statsPanel-${n}`;if(i)p.hidden=true;analytics.appendChild(p)});
  const overview=document.getElementById('statsPanel-overview');overview.classList.add('stats-overview-stack');
  const kpi=document.getElementById('kpiGrid');overview.appendChild(kpi);
  const two=analytics.querySelector('.analytics-two');
  if(two){const progress=two.children[1];if(progress){progress.classList.add('stats-progress-card');overview.appendChild(progress)}const legacy=document.createElement('div');legacy.className='stats-hidden-legacy';while(two.firstChild)legacy.appendChild(two.firstChild);two.replaceWith(legacy)}
  const timeline=document.getElementById('timelineHeat')?.closest('.card');if(timeline)overview.appendChild(timeline);

  const monthly=document.querySelector('.page[data-page="monthly"]');
  if(monthly){
    const yearWall=monthly.querySelector('.year-wall');if(yearWall)document.getElementById('statsPanel-year').appendChild(yearWall);
    [...monthly.children].forEach(ch=>document.getElementById('statsPanel-month').appendChild(ch));
    monthly.dataset.page='legacy-monthly';monthly.hidden=true;
  }
  moveEvolutionCard();
  tabs.onclick=e=>{const b=e.target.closest('[data-stats-tab]');if(!b)return;showStatsTab(b.dataset.statsTab)};
  new MutationObserver(()=>moveEvolutionCard()).observe(analytics,{childList:true});
}
function moveEvolutionCard(){
  const card=document.getElementById('readingEvolutionCard'),p=document.getElementById('statsPanel-evolution');if(card&&p&&card.parentElement!==p)p.appendChild(card);
}
...
function rebuildBottomNav(){
  ...
  nav.innerHTML='<button class="nav-btn active" data-go="today"><span>⌂</span>首页</button><button class="nav-btn" data-go="calendar"><span>▦</span>日历</button><button class="nav-btn" data-go="library"><span>▤</span>书库</button><button class="nav-btn" data-go="notes"><span>✎</span>笔记</button><button class="nav-btn" data-go="analytics"><span>◫</span>统计</button>';
