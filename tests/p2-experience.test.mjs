...
assert.match(app,/sourceArchives=\{/,'cross-source merges must preserve independent source records');
...
assert.match(features,/card\?\.id===['"]monthCalendarWrap['"]\?preparedCalendarPng/,'month calendar preview must use its prepared PNG renderer');
assert.match(features,/renderCalendarPng=async function[\s\S]*renderMonthCalendar/,'calendar export must rebuild the current month before capture');
assert.match(features,/renderMonthlyPng/,'monthly reports must use a dedicated PNG renderer');
assert.match(features,/openModulePreview\(null,[\s\S]*preparedMonthlyPng\)/,'the monthly export action must rebuild the report before capture');
