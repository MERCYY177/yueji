...
assert.match(evolutionDate,/weReadLastRead\|\|b\.weReadShelfReadUpdate/,'the WeRead view may use a shelf activity timestamp only as an explicitly approximate evolution trace');
...
const itemStart=core.indexOf('function traceDate');
...
const evolutionState={books:[{key:'merged:1',sources:['moon','weread'],weReadSeconds:7200},{key:'weread:trace',sources:['weread'],weReadSeconds:3600,weReadShelfReadUpdate:'2026-09-05'},{key:'manual:1',sources:['manual']}],sessions:[{bookKey:'merged:1',source:'moon',date:'2026-09-01',minutes:30},{bookKey:'merged:1',source:'manual',date:'2026-09-03',minutes:99}],journals:{j:{bookKey:'manual:1',read:true,date:'2026-09-04'}}};
...
assert.equal(evolutionContext.weread.length,2,'WeRead view must retain both verified changes and clearly labelled recent-activity traces');
...
assert.equal(evolutionContext.weread.find(x=>x.b.key==='weread:trace').evidenceQuality,'recent-activity','a shelf activity date must stay in WeRead as an approximate trace');
