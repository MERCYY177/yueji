...
assert.deepEqual(
  [...dates({ weReadSnapshots: [{ date: '2026-09-09', progress: 42, seconds: 9000 }] })],
  [],
...
assert.deepEqual(
  ...
  ['2026-09-10'],
  'a later increase is a verified reading event',
);
assert.deepEqual(
  [
    ...dates({
...
assert.deepEqual(
  [
    ...dates({
      weReadSnapshots: [{ date: '2026-09-10', progress: 42, seconds: 9000, activity: true }],
    }),
  ],
  ['2026-09-10'],
  'same-day growth retained by the sync merger is accepted',
);

const features = fs.readFileSync(new URL('../yueji-features.js', import.meta.url), 'utf8');
assert.doesNotMatch(
  features,
  /const d=b\.weReadLastRead\|\|b\.finishedDate/,
  'annual read report must not use the shelf/import timestamp',
);
assert.match(
  features,
  /yuejiVerifiedWeReadActivityDates/,
...
const yearCalendar =
  app.match(
    /function renderYearCalendar\(\)[\s\S]*?\$\('yearReadDays'\)\.textContent\s*=\s*count;?\s*\}/,
  )?.[0] || '';
...
assert.match(
  ...
  'year choices must be derived from stored reading data',
);
assert.match(
  app,
  /state\.sessions\.forEach\(\(?s\)?\s*=>\s*add\(s\.date\)\)/,
  'imported session years must populate the selector',
);
...
assert.match(
  ...
  'calendar arrows must navigate the available data years',
);
assert.match(
  app,
  /target\s*=\s*years\[index\s*-\s*direction\]/,
  'year arrows must move through data-derived years instead of empty years',
);
assert.match(
  app,
  /prev\.disabled[\s\S]*?years\.length\s*-\s*1/,
  'year navigation must stop at the oldest available data year',
);
assert.match(
  features,
  /function syncYearReportOptions\(\)/,
  'annual report must use the same data-derived year choices',
);
assert.doesNotMatch(
  features,
  /yearWallYear[^\n]+type="number"/,
  'annual report year must not rely on manual number entry',
);

const yearFn = app.match(
  /function calendarDataYears\(\)\s*\{[\s\S]*?return \[\.\.\.years\][\s\S]*?\n\}/,
)?.[0];
