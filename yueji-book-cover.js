(() => {
  ...
  async function normalizeImage(file) {
    ...
    bmp.close?.();
    const blob = await new Promise((resolve) => c.toBlob(resolve, 'image/webp', 0.9));
    return blob || file;
  }

  function installEditor() {
    const sheet = document.getElementById('bookSheet'),
      note = document.getElementById('bookDataNote');
    if (!sheet || !note || document.getElementById('bookCoverEditor')) return;
    const box = document.createElement('div');
    box.id = 'bookCoverEditor';
    box.className = 'book-cover-editor';
    box.innerHTML = `
  ...
  async function refreshPreview(key) {
    const wrap = document.getElementById('bookCoverPreview');
    if (!wrap) return;
    ...
    const btn = document.getElementById('restoreBookCover');
    if (btn) btn.hidden = !b.manualCover;
  }

  function wrapOpenBook() {
    if (window.__yuejiBookCoverOpenWrapped) return;
    window.__yuejiBookCoverOpenWrapped = true;
    const original = openBook;
    openBook = function (key) {
      original(key);
      setTimeout(() => refreshPreview(key), 0);
    };
  }
